...
import { labs, articles } from "../../../data/mock";
...
export default async function LabDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { items } = labs;
  const lab = items.find((n) => n.id === id);
  if (!lab) {
    return (
      <main className="min-h-dvh bg-background text-foreground"><div className="mx-auto max-w-5xl px-6 md:px-10 py-12">Lab not found</div></main>
    );
  }
  const avg = averageScores(lab.topArticles.map((a) => a.id));
  ...
  return (
    <main className="min-h-dvh bg-background text-foreground">
      <div className="mx-auto max-w-7xl px-6 md:px-10 py-12 md:py-16">
        ...
        <h2 className="mt-10 text-sm font-semibold tracking-tight">Activity over time</h2>
        <section className="mt-3 grid grid-cols-1 md:grid-cols-12 gap-8">
          ...
          <aside className="md:col-span-4 self-stretch rounded-2xl border border-foreground/10 p-5 bg-foreground/[.02] h-full min-h-[380px] lg:min-h-[420px] flex flex-col items-center justify-center text-center gap-4">
            <h2 className="text-sm font-semibold tracking-tight">Lab Performance Metrics</h2>
        ...
        <section className="mt-12">
          <h3 className="text-sm font-semibold tracking-tight">Related Projects</h3>
          <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
            {lab.topArticles.map((a) => (
